...
#define NUM rand() % 20
#define CON_CG COORD cord; HANDLE h = GetStdHandle(STD_OUTPUT_HANDLE)
#define POS(a, b) cord.X = a; cord.Y = b; SetConsoleCursorPosition(h, cord); 
...
void mostrar_matriz()
{
    ...
    cout << "-----------------------------------------\nTamanho: " << pontos << endl << "Recorde: " << record << endl;    
}
...
int lose()
{
    ...
    CON_CG;
    if(nRecord){
        POS(60, 11); cout << "  Novo Recorde!, Parabéns!";
        nRecord = false;
        verificar_recorde();
    }
    else{
        POS(60, 11); cout << "       Você Perdeu!!";
    }
    POS(60, 12); cout << "Deseja Jogar Novamente?(s/n)";
    POS(60, 13); int esc; esc = getch();
    if(esc == 's'){
        choice = ' ';
        pontos = 1;
        POS(60, 11); cout << "                                ";
